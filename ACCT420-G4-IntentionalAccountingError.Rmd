...
### Post-processing of Logistics Regression
```{r, fig.height=20, fig.width=12, echo = F}
...
After running the post-processing code above, the AUC score improved to **0.68785**
...
Similarly to previous models, we did post processing on the best model (Elastic Net) as outlined previously to improve the model.

The results then improved as the Out-of-sample AUC improved to **0.67173**
...
The cross validation AUC score of the model is **0.63151** 
...
<INSERT THE SCORE OF THE MODEL HERE>
...
Similar to other models, we did a post processing technique as previously outlined. 
...
There are a few factors that we want to discuss in choosing our final model: metric performance, complexity , model explainability and interpretability. To enable easier comparison, the summary of results table after post-processing are given below:

+-----------------------+-------------------+
| Method                | Out-of-sample AUC |
:+=====================:+:=================:+
| Logistics Regression  | 0.68785	    |
+-----------------------+-------------------+
| Elastic Net           | 0.67173	    |
+-----------------------+-------------------+
| LightGBM              | 0.63151	    |
